...
LCD.prototype.printHeatingStatus = function (status) {
  console.log('FINER: lcd.printHeatingStatus');
  var self = this;
  self.checkInit();

	return self.setCursor(0,2).print('HEAT: ' + status == 1 ? 'OFF' : ' ON');
}
...
LCD.prototype.printAvgTemperature = function (temp) {
  ...
  var s = new Date().toString().substring(16, 21);
	return self.setCursor(0,1).print(s + ' C ' + Math.round(temp * 100) / 100);
}
